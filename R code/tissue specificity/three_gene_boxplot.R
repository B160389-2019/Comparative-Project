rm(list = ls())
setwd("/Users/yaoyuelin/Downloads/desktop/Comparative/comparative_file/initial_dataset_cattle")
...
gene_list<-c("CLSTN3","ADAM7","TDRD1")
...
expression_data$expression<-log10(expression_data$expression+1)
expression_data<-expression_data[-1,]
tiff(file="/Users/yaoyuelin/Desktop/three_genes.tiff",width = 2500,height = 1600,res = 300)
p<-ggplot(expression_data, aes(x=tissue, y=expression, fill=Species,palette = "npg"))+geom_boxplot(position=position_dodge(1),outlier.shape = NA)+scale_fill_manual(values=c("blue", "red"))+
  theme(axis.text.x = element_text(angle = 45, hjust = 1,size =18 ,colour = "black"),panel.grid=element_blank(),panel.border=element_blank(),axis.line=element_line(size=1,colour="black"),axis.title =element_text(size =25,colour = "black" ),axis.text.y = element_text(size =15,colour = "black" ),legend.text = element_text(size =15,colour = "black" ) ,legend.title =element_text(size =20,colour = "black" ),panel.background = element_rect(fill='transparent', color=NA) )+ylab("Expression")+xlab(" ")
p+facet_wrap( ~gene,scales = "free_y",ncol=1)+theme(
  strip.background = element_rect(color = NA,fill=NA,
                                  size=1),strip.text = element_text(face="bold.italic",size =14)
  )
