...
###########using Seurat to correct batch effect
...
tiff(file = "~/TSNE_1.tiff",##reqiured to change
...
tiff(file = "~/TSNE_2.tiff",##reqiured to change
     res = 300, width = 3000, height = 2000,compression = "lzw")
par(mar=c(2,2,5,5))
info.norm<-as.data.frame(expression_tsne$Y)
colnames(info.norm)<-c("tsne1","tsne2")
...
ggplot(info.norm, aes(x = tsne1, y = tsne2, colour = experiment$tissue_new)) + 
  ...
  theme(panel.background=element_blank(),legend.title=element_blank(),legend.text = element_text(size=20),
        ...
        axis.text.x=element_text(size=22,color="black",hjust=0.95,vjust=0.2))+
  scale_colour_manual(name="Tissue_categories",values=colors)#+stat_ellipse(type="norm")
dev.off()
...
library(pheatmap)
pheatmap(corr_mat,file="/Users/yaoyuelin/Desktop/coormat_median.pdf",cluster_rows = T,cluster_cols = T,fontsize_row = 15,fontsize_col = 15,cellwidth = 13,cellheight = 13)
...
dim(corr_mat)
head(corr_mat[1:10,1:10])
tiff(file = "/Users/yaoyuelin/Desktop/MAD.tiff",##reqiured to change
     res = 300, width = 4000, height = 4000,compression = "lzw")
corrplot(corr_mat,method="color",order = "hclust",tl.col = "black",tl.cex = 1.8,cl.cex = 1.8,col = brewer.pal(n = 11, name = "RdYlBu"),type = "upper",is.corr = F)
dev.off()



pheatmap(corr_mat,file="/Users/yaoyuelin/Desktop/coormat_mad.pdf",cluster_rows = T,cluster_cols = T,fontsize_row = 15,fontsize_col = 15,cellwidth = 13,cellheight = 13)
...
dim(corr_mat)
head(corr_mat[1:10,1:10])
tiff(file = "/Users/yaoyuelin/Desktop/heatmap1.tiff",##reqiured to change
     res = 300, width = 5000, height = 1500,compression = "lzw")
corrplot(corr_mat,method="color",cex.axis=0.5,outline = T,order="hclust",addrect=10,col = colorRampPalette(c("darkred","white","midnightblue"))(100) )
dev.off()


library(pheatmap)
pheatmap(corr_mat,file="/Users/yaoyuelin/Desktop/coormat_mean.tiff",cluster_rows = T,cluster_cols = T,fontsize_row = 15,fontsize_col = 15,cellwidth = 13,cellheight = 13,res=300)
