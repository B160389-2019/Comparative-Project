...
median_tissue_tpm<-aggregate(expression,list(as.factor(experiment$annotation)),median) ## also calculated mad/ variance with the same script
median_tissue_tpm
...
for (i in 1:length(tissue)){
  ...
  cattle_tissue<-sort(median_tissue_tpm[cattle_name,],decreasing = T)
...
tiff(file = "~/median.tiff",##reqiured to change
     res = 300, width = 6500, height = 1900,compression = "lzw")
par(mar=c(8,8,6,1))
my<-barplot(Results1_percent,beside = T,col=col,cex.axis = 1.8,cex.names = 1.8,ylim = c(0,0.7))
title(ylab="Percentage of shared orthologs", xlab = "Windows of genes sorted by median expression",line=4, cex.lab=2.5)
...
Results1 <- array(NA,dim = c(length(tissue),7))
experiment$annotation<-paste(experiment$tissue_new,experiment$species,sep="-")
median_tissue_tpm<-aggregate(expression,list(as.factor(experiment$annotation)),mad)
median_tissue_tpm
rownames(median_tissue_tpm)<-median_tissue_tpm[,1]
median_tissue_tpm<-median_tissue_tpm[,-1]
median_tissue_tpm[,1:10]
...
col<-c("#00008B","#008B8B","#8B008B","#8B0000","#828282","#00B2EE","#0000FF","#FF0000","#FF00FF","#FF8C00","#FFFF00","#006400","#00FF00",
       "#CD950C","#FF6A6A","#CCCC33","pink","#FF1493","#8B7355","#00F5FF")
tiff(file = "/Users/yaoyuelin/Desktop/Overlapped_gene_percentage_mean.tiff",##reqiured to change
